...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (atac_.IsCollision(aabb1,aabb2)) {
			color = 0xFF0000ff;
		}
		else {
			color = 0xFFFFFFFF;
		...
		atac_.Draw(aabb1, worldViewProjectionMatrix, viewPortMatrix,WHITE);
		atac_.Draw(aabb2, worldViewProjectionMatrix, viewPortMatrix, color);
		///
		/// ↑描画処理ここまで
		///
